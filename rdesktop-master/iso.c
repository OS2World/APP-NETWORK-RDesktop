...
{debugCP( "tcp_connect() - false" );
		return False;
}
debugCP( "iso_send_connection_request()..." );

	iso_send_connection_request(username, neg_proto);

debugCP( "iso_recv_msg()..." );
	s = iso_recv_msg(&code, NULL);
	if (s == NULL)
{debugCP( "iso_recv_msg() - false" );
		return False;
}
...
debugCP( "tcp_disconnect()..." );
		...
		{
debugCP( "tcp_tls_connect()..." );
			if (!tcp_tls_connect())
			{
debugCP( "tcp_tls_connect() - false" );
...
debugCP( "goto retry" );
			...
			g_encryption = False;
debugCP( "tcp_tls_connect() - true, Connection established using SSL" );
