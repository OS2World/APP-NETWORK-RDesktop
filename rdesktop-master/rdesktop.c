...
int g_pos = 0;			/* 0 position unspecified,
				   ...
				   2 xpos neg,
				   4 ypos neg  */
/* OS/2: 8 report main window position on exit,
         => position unspecified when (g_pos & 7) == 0 */
...
#else
					g_ypos = strtol(p, &p, 10);
				}

        } // if ( optarg[0] == '#' ), else

        // A new feature: print position of the main window on exit:
        // #winpos=nnn,nnn - position relative top-left corner of the desktop.
				if ( memicmp( p, "#winpos", 7 ) == 0 )
					g_pos |= 8;
#endif
			...
			case 'X':
#if defined(__OS2__)
				g_embed_wnd = strtoul(optarg, NULL, 0);
#else
...
debugCP();
	...
	if (g_codepage[0] == 0)
	{
		if (setlocale(LC_CTYPE, ""))
		{
			...
			debug( "Use as local codepage (nl_langinfo()): %s", g_codepage );
		}
		else
		{
			STRNCPY(g_codepage, DEFAULT_CODEPAGE, sizeof(g_codepage));
			debug( "Use as local codepage (DEFAULT_CODEPAGE): "
                   DEFAULT_CODEPAGE );
		}
	}
#endif
