...
#define TCP_CLOSE(_sck) soclose(_sck)
...
tcp_send(STREAM s)
{
	...
	while (total < length)
	{
...
#ifdef __OS2__
						 ssl_err == SSL_ERROR_WANT_WRITE) && tcp_can_send(g_sock, 250) )
				{
#else
		...
		else
		{
			...
			{
#ifdef __OS2__
				if (sent == -1 && TCP_BLOCKS && tcp_can_send(g_sock, 250))
				{
#else
...
tcp_connect(char *server)
{
	...
	option_value = 1;
#ifdef __OS2__
	ioctl( g_sock, FIONBIO, (char *)&option_value );
#else
...
#ifdef __OS2__
  {
    // :-/ Socket should be in blocked mode before close for any system.
    char option_value = 0;
    ioctl( g_sock, FIONBIO, (char *)&option_value );
    shutdown( g_sock, 1 );
  }
